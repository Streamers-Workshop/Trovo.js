...
module.exports = class Client extends EventEmitter {
  ...
  async login(url, email, password, cookies) {
    ...
    this.loadCookies(cookies, url)
    await this.page.waitForTimeout(10000);

    if (email && email.length > 0 && cookies.length === 0)  {
      ...
      const error_line = "div.login-box > div.content-box > div.content-left > div > div:nth-child(4) > div.input-box.border-bottom > div.input-line.input-line-error";
      const error_element = "div.login-box > div.content-box > div.content-left > div > div:nth-child(4) > div.tips-wrapper > p";


      const verifyEmail = "body > div.login-box > div.content-box > div.content-left > div > section > p";

      await this.page.waitForTimeout(3000);

      if (await this.page.$(error_line) !== null) {
        const error_message = await this.page.evaluate(element => element.textContent, await this.page.$(error_element));
        this.logger(`\nTrovo Account Error - ${error_message}`);
      } else if(await this.page.$(verifyEmail) !== null) {
        const verify_message = await this.page.evaluate(element => element.textContent, await this.page.$(verifyEmail));
        this.logger(`Verify Email: ${verify_message}`);
	      this.loggedin = true;
      }
    }
  }

  async getCookies(strCookies) {
    const cookiesObj = await this.page.cookies();
    fs.writeFileSync(strCookies, JSON.stringify(cookiesObj));
    return cookiesObj;
  }

  async loadCookies(cookies, url) {
    if( cookies.length > 0 ){
      ...
      await this.page.goto(url);
      this.loggedin = true;
      this.logger("Loaded cookies from file...");
    }
  }
