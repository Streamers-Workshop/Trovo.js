...
module.exports = class Client extends EventEmitter {
  constructor(settings) {
    ...
    if (settings) {
      this.headless =
        settings.headless === undefined ? this.headless : settings.headless;
      this.agent = settings.agent === undefined ? this.agent : settings.agent;
      this.viewport =
        settings.viewport === undefined ? this.viewport : settings.viewport;
    }
    this.page = null;
  ...
  async login(url, email, password) {
    ...
    if (email) {
      ...
      const loginXpath =
        'div > nav > div.uinfo-wrapper.flex > button:nth-child(4)';
      ...
      const loginButtonPath =
        'div.login-box > div.content-box > div.content-left > div > button';
  ...
  async _sendMessage(message) {
    ...
    */
    ...
    await this.page.waitFor(
      'div > div > div.live-wrap.base-main > div > div.chat-wrap.open > div.chat-list > section > div.input-container > div.input-wrap.flex.justify-between.align-end > div',
    );
    ...
    const inputPath =
      'div > div > div.live-wrap.base-main > div > div.chat-wrap.open > div.chat-list > section > div.input-container > div.input-wrap.flex.justify-between.align-end > div > div.editor';
    await this.page.type(inputPath, message);

    // Click the Send Button, since yeah. You need todo this LoL
    const buttonPath =
      'div > div > div.live-wrap.base-main > div > div.chat-wrap.open > div.chat-list > section > div.input-container > div.input-feature-box.align-center.justify-between > button.cat-button.btn-send.normal.primary';
    ...
    const closePath =
      'div > div > div.live-wrap.base-main > div > div.chat-wrap.open > svg.svg-icon.cursor-pointer.toggle-icon.hover-highlight';
  ...
  decodeBinaryMessage(data) {
    ...
    const decodeStruct = {
      ...
      packet: binary.slice(
        binary.readUInt16BE(8) === 8 ? 18 : 22,
        binary.readUInt32BE(0),
      ),
    };
