...
module.exports = class Client extends EventEmitter {
  ...
  async login(url, email, password) {
      ...
      try {
        ...
        if (output) {
          if (output.data.length > 1) {
            ...
            if (output.data[0].chatType != undefined) {
              console.log("ae")
              if (output.data[0].__history__ != undefined) return;
              switch (output.data[0].chatType) {
                case 5004: // User Joined
                    this.emit("chatEvent", "userJoined", output.data[0]);
                  break;
                case 5003: // User Followed
                    this.emit("chatEvent", "userFollowed", output.data[0]);
                  break;
                case 5001: // User Subbed
                    this.emit("chatEvent", "userSubbed", output.data[0]);
                  break;
                case 5: // Gift
                    this.emit("chatEvent", "giftRecieved", output.data[0]);
                  break;
                default:
                  this.emit("chatMessage", output.data[0]);
              }
            } else {
              this.emit("chatMessage", output.data[0]);
            }
    ...
    if (email) {
      ...
      await this.page.waitFor('div > nav > div.uinfo-wrapper.flex > button:nth-child(4)');
