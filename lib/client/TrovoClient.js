          ...
          if (output.data.length > 1) {
            ...
            if (output.data[0].chatType != undefined) {
              ...
              switch (output.chatType) {
                case 5004: // User Joined
                    this.emit("chatEvent", "userJoined", output[0]);
                  break;
                case 5003: // User Followed
                    this.emit("chatEvent", "userFollowed", output[0]);
                  break;
                case 5001: // User Subbed
                    this.emit("chatEvent", "userSubbed", output[0]);
                  break;
                case 5: // Gift
                    this.emit("chatEvent", "giftRecieved", output[0]);
                  break;
                default:
                  this.emit("chatMessage", output[0]);
              }
            } else {
              this.emit("chatMessage", output[0]);
            }
